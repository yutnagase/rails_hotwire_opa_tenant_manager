...
| #   | 画面             | パス                              | 説明                                         |
| --- | ---------------- | --------------------------------- | -------------------------------------------- |
| 1   | プロジェクト一覧 | `/projects`(ルート)               | テナント内の全プロジェクトを一覧表示（admin/memberはCRUD可） |
| 2   | プロジェクトフォーム | `/projects/new`, `/projects/:id/edit` | プロジェクトの作成・編集（admin/member）  |
| 3   | タスク一覧       | `/projects/:project_id/tasks`     | プロジェクト配下のタスク一覧（admin/memberはCRUD可） |
| 4   | タスク詳細       | `/projects/:project_id/tasks/:id` | タスク詳細・編集・ステータス更新             |
| 5   | タスクフォーム   | `/projects/:project_id/tasks/new`, `/projects/:project_id/tasks/:id/edit` | タスクの作成・編集（admin/member） |
| 6   | ユーザー管理     | `/admin/users`                    | ユーザー一覧とロール管理                     |
| 7   | テナント設定     | `/settings`                       | テナント名の表示・編集（adminのみ）          |
...
- `can?(action, resource)`ビューヘルパーで権限に応じたUI表示を制御する

権限マトリクス（リソース別）:

| リソース \ ロール     | admin | member | guest |
| --------------------- | ----- | ------ | ----- |
| tenant (read)         | ○     | ○      | ○     |
| tenant (update)       | ○     | ×      | ×     |
| project (read)        | ○     | ○      | ○     |
...
| GET      | /projects/new                        | projects#new      | プロジェクト作成フォーム |
| POST     | /projects                            | projects#create   | プロジェクト作成     |
| GET      | /projects/:id/edit                   | projects#edit     | プロジェクト編集フォーム |
| PATCH    | /projects/:id                        | projects#update   | プロジェクト更新     |
| DELETE   | /projects/:id                        | projects#destroy  | プロジェクト削除     |
| GET      | /projects/:project_id/tasks          | tasks#index       | タスク一覧           |
| GET      | /projects/:project_id/tasks/new      | tasks#new         | タスク作成フォーム   |
| POST     | /projects/:project_id/tasks          | tasks#create      | タスク作成           |
| GET      | /projects/:project_id/tasks/:id      | tasks#show        | タスク詳細           |
| GET      | /projects/:project_id/tasks/:id/edit | tasks#edit        | タスク編集フォーム   |
| PATCH    | /projects/:project_id/tasks/:id      | tasks#update      | タスク更新           |
| DELETE   | /projects/:project_id/tasks/:id      | tasks#destroy     | タスク削除           |
| GET      | /admin/users                         | admin/users#index | ユーザー一覧         |
| PATCH    | /admin/users/:id                     | admin/users#update| ユーザーロール更新   |
| GET      | /settings                            | settings#show     | テナント設定         |
| PATCH    | /settings                            | settings#update   | テナント設定更新     |
...
│   ├── controllers/
│   │   ├── admin/
│   │   │   └── users_controller.rb            # ユーザー管理(admin)
│   │   ├── concerns/
│   │   ├── users/
...
│   │   ├── projects_controller.rb     # フルCRUD
│   │   ├── settings_controller.rb     # テナント設定
│   │   └── tasks_controller.rb        # フルCRUD
│   ├── helpers/
│   │   └── application_helper.rb  # can?(action, resource)ヘルパー
│   ├── models/
...
│       ├── admin/
│       │   └── users/
│       │       └── index.html.erb          # ユーザー一覧・ロール管理
│       ├── layouts/
│       │   └── application.html.erb
│       ├── projects/
│       │   ├── _form.html.erb              # プロジェクトフォームパーシャル
│       │   ├── edit.html.erb
│       │   ├── index.html.erb
│       │   └── new.html.erb
│       ├── settings/
│       │   └── show.html.erb               # テナント設定
│       └── tasks/
│           ├── _form.html.erb              # タスクフォームパーシャル
