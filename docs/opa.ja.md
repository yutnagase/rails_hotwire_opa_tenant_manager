...
認可ポリシーはリソースごとに権限を定義している。各リソース（tenant, project, task, user）に個別のルールを持つ。
...
`ApplicationHelper`の`can?(action, resource)`ヘルパーで、権限に応じたUI表示を制御する。
...
ビューではこのヘルパーを使い、ユーザーのロールに応じてCRUD操作のUI要素（「新規作成」「編集」「削除」ボタンなど）の表示を切り替えている。
